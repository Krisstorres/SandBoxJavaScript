...
const tablas_gt = path.join(__dirname,'./tablas_GT.csv');
const nombreContatJson= path.join(__dirname,'./nameList.json')
const jsonFile = path.join(__dirname,'/archivo.json');
...
async function realizarFiltro() {
    try {
        ...
        //
        const jsonData = await fs.promises.readFile(nombreContatJson, 'utf8');
        const jsonObject = JSON.parse(jsonData);
...
async function filtarDatos(){
    ...
    const nombresObj= JSON.parse(nombresExtraidos);
    ...
    console.log('Cantidad de coincidencias encontradas = ' + i);
    return uniqueTables;
    
};

async function queryTester(qry){
const querry =qry ;
const resultadoQuery1 = [];
const almacenamiento1=[];
const almacenamiento2=[];

const result = await filtarDatos();
var cont=0
result.forEach(cols => {
    console.log('Tabla completa ' + JSON.stringify(cols));
    if (cols.TABLE_NAME === querry) {
        if (!cols.COLUMN_NAME.includes('I3_')) {
            if(! almacenamiento1.some(col => col==cols.COLUMN_NAME )){
            almacenamiento1.push(cols.COLUMN_NAME)
            cont+=1;
            resultadoQuery1.push({
                COLUMN_NAME: cols.COLUMN_NAME,
                DATA_TYPE: cols.DATA_TYPE,
                DATA_LENGTH: parseInt(cols.DATA_LENGTH, 10) 
            });
            }
        } else {
            console.log('La columna contiene I3 ! ');
        }
    }
});

return resultadoQuery1;
};
const test_a=await queryTester('SATIS_TIENDAS_CR');
console.log(test_a);